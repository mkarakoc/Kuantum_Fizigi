...
% Turkish babel'in "=" shorthand'ını kapat
...
\end{picture} \\ Prof. Dr. Mesut Karakoç}
...
%%
\begin{equation}
P(x,t) = |\psi_1 + \psi_2|^2 = |\psi_1 |^2 + |\psi_2|^2 + 2\text{Re}(\psi_1  \psi_2^*)
...
\end{equation}
%%
Bir parçacığın konumu için beklenen değeri ve standart sapmayı bulmak kolaydır, $f(x)$ yerine $\hat x$ (konum operatörünü) koymak yeterlidir. 
...
%%
\begin{align}
\langle \hat p \rangle &= m \frac{d}{dt} \int ^{\infty }_{-\infty }dx \psi^{\ast }\left(x,t\right) \hat x \psi \left( x,t\right)\\
&=m\int ^{\infty }_{-\infty }dx\left( \dfrac {\partial \psi ^{\ast }\left( x,t\right) }{\partial t} \hat x\psi \left( x,t\right) +\psi ^{\ast }\left( x,t\right) \hat x \dfrac {\partial \psi \left( x,t\right) }{\partial t}\right) \nonumber 
\end{align}
%%
%% https://en.wikipedia.org/wiki/Ehrenfest_theorem
%% Schrödinger picture vs Heisenberg picture
%% https://physics.stackexchange.com/a/412514
%% <dx / dt> Equals zero in the Schrödinger picture (which is the one you seem to be working with)
%% because the operators in the Schrödinger picture are constant. That is, they don't depend explicitly %% on time. What changes in time is their expected value, and this is represented by:
%%
yazılabilir. Schrödinger resminde operatörler sabit olduğudan bu hesapta $d \hat x/dt$ türev ifadesinin yer almadığına dikkat edilmelidir. Zamanla değişen operatörlerin beklenen değerleridir çünkü zamana bağlı olan ifade $\psi(x,t)$ ve onun eşleniğidir. Dalga fonksiyonunun zamanla değişimi $\langle \hat x \rangle$'deki zamana bağlı değişimi belirler. Olasılık akısını hesaplarken yaptığımız gibi yukarıdaki denklemde Schrödinger denklemini ve eşleniğini kullanalarak,
%%
\begin{equation}
\langle \hat p\rangle =\dfrac {\hbar }{2i}\int ^{\infty }_{-\infty }dx\left( \dfrac {\partial ^{2}\psi ^{\ast }}{\partial x^{2}}\hat x\psi -\psi ^{\ast } \hat x\dfrac {\partial ^{2}\psi }{\partial x^{2}}\right)
...
%%
\begin{align*}
\dfrac {\partial }{\partial x}\left( \dfrac {\partial \psi ^{\ast }}{\partial x} \hat x\psi \right) =\dfrac {\partial ^{2}\psi^{\ast }}{\partial x^{2}}\hat x\psi +\dfrac {\partial \psi ^{\ast }}{\partial x}\psi +\dfrac {\partial \psi ^{\ast }}{\partial x}\hat x\dfrac {\partial \psi }{\partial x} \\
\dfrac {\partial }{\partial x}\left( \psi ^{\ast }\hat x\dfrac {\partial \psi }{\partial x}\right) =\dfrac {\partial \psi ^{\ast }}{\partial x}\hat x\dfrac {\partial \psi }{\partial x}+\psi ^{\ast }\dfrac {\partial \psi }{\partial x} + \psi ^{\ast } \hat x \dfrac {\partial ^{2}\psi }{\partial x^{2}}
\end{align*}
%%
Yukarıdaki iki eşitlikten alttaki üsttekinden çıkarılırsa,
%%
\begin{equation*}
\left( \dfrac {\partial ^{2}\psi ^{\ast }}{\partial x^{2}}\hat x\psi -\psi ^{\ast }\hat x\dfrac {\partial ^{2}\psi }{\partial x^{2}}\right) = \dfrac {\partial }{\partial x}\left( \dfrac {\partial \psi ^{\ast }}{\partial x}\hat x\psi -\psi ^{\ast } \hat x\dfrac {\partial \psi }{\partial x}\right) +\psi ^{\ast }\dfrac {\partial \psi }{\partial x}-\dfrac {\partial \psi ^{\ast }}{\partial x}\psi 
\end{equation*}
%%
...
%%
\begin{equation*}
\langle \hat p\rangle =\dfrac {\hbar }{2i}\int ^{\infty }_{-\infty }dx\left( \dfrac {\partial ^{2}\psi ^{\ast }}{\partial x^{2}} \hat x\psi -\psi ^{\ast } \hat x\dfrac {\partial ^{2}\psi }{\partial x^{2}}\right) =  \dfrac {\hbar }{2i}\int ^{\infty }_{-\infty }dx \left[\dfrac {\partial }{\partial x}\left( \dfrac {\partial \psi ^{\ast }}{\partial x} \hat x\psi -\psi ^{\ast } \hat x\dfrac {\partial \psi }{\partial x} + \psi^\ast \psi\right) +2\psi ^{\ast }\dfrac {\partial \psi }{\partial x}\right]
\end{equation*}
%%
sonucuna ulaşılır. Bu ifadeyi biraz daha açarsak,
%%
\begin{equation*}
\langle \hat p\rangle = \dfrac {\hbar }{2i}\left( \dfrac {\partial \psi ^{\ast }}{\partial x}\hat x\psi -\psi ^{\ast } \hat x\dfrac {\partial \psi }{\partial x} + \psi^\ast \psi \right){\bigg |}_{-\infty}^{\infty} + \int ^{\infty }_{-\infty }dx \psi ^{\ast }\left(\dfrac {\hbar }{i}\dfrac {\partial}{\partial x}\right) \psi
\end{equation*}
%%
...
%%
\begin{equation}
\langle \hat p\rangle =  \int ^{\infty }_{-\infty }dx \psi(x,t) ^{\ast }\left(\dfrac {\hbar }{i}\dfrac {\partial}{\partial x}\right) \psi(x,t)
