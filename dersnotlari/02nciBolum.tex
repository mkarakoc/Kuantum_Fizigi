...
Kuantum fiziğinde olasılık kavramı klasik fiziktekinden farklıdır. Örneğin bir madeni paranın fırlatılmasıyla yazı mı, tura mı geleceğini bilememizin nedeni klasik fizikte sistem hakkındaki bilgimizin yetersizliği olarak belirtilir (örneğin ortamdaki hava moleküllerinin tek tek bireysel olarak para yüzeyi ile aralarındaki etkileşmenin tam bir modelinin olmaması v.b. gibi). Fakat kuantum fiziğinde ihtimaliyet bilgi eksikliğinden değil, dalga fonksiyonu hakkında bilebileceklerimizin temel bir sınırlamasından kaynaklanır.
