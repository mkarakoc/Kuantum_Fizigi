...
%%
\begin{align}
	{\int\limits_{-a}^{a} d x\left(\frac{1}{\sqrt a}e^{-i k x}\right)^{*} \frac{1}{\sqrt a}e^{i k x}=0} \text{ \quad ve \quad }
...
Serbest parçacığın veya sınırları $-a/2$ ve $+a/2$ aralığında olan bir kuyu içindeki parçacığın $\sin(kx)$ ve $\cos(kx)$ özfonksiyonlarıyla tanımlanabildiğini gördük. Bu özfonksiyonlar $x \rightarrow -x$ dönüşümü altında \emph{tek} veya \emph{çift} fonksiyonlardır.

Herhangi bir $\psi(x)$ fonksiyonu aşağıdaki gibi tek ve çift fonksiyonların toplamı olarak yazılabilir,
