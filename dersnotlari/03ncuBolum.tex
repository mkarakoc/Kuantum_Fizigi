...
= \int _ { - \infty } ^ { \infty } d p \phi ( p ) \frac { e ^ { i p x \hbar \hbar } } { \sqrt { 2 \pi \hbar } }
...
\end{equation}
%%
olarak yazılır. Bu diferansiyel denklemin çözümleri $e ^ { i k x }$, $e ^ { - i k x }$ veya $\sin kx$, $\cos kx$ fonksiyonlarının lineer kombinasyonları olacaktır. Örneğin;
%%
\begin{equation}
u(x) = A e ^ { i k x } + B e ^ { - i k x }
\end{equation}
gibi bir çözüm önerilebilir. Fakat bu çözümün problemi karesi integrallenebilir olmamasıdır.
%%
\begin{align}
\int _ { - \infty } ^ { \infty } d x \left|u(x) \right| ^ { 2 } 
&= \int _ { - \infty } ^ { \infty } d x \left(A^2 + B^2 + AB\left(e ^ { -i 2k x } + e ^ {  i 2k x }\right) \right)\\
&= \int _ { - \infty } ^ { \infty } d x \left(A^2 + B^2 + 2AB\cos 2kx \right)\\
&= \left[\left(A^2 + B^2\right)x + \frac{AB}{k}AB\sin 2kx \right]_{-\infty}^\infty
\end{align}
%%
Çünkü yukarıdaki işlemlerin sonucundan anlaşılacağı üzere integral ıraksar (patlar). Dolayısıyla bu tür sistemlerin dalga fonksiyonlarını normalize etmenin başka yolları önerilmiştir.
%%
\begin{enumerate}
	\item 1
