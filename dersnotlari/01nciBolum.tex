...
Bir zamanlar denildiği gibi; ``Araştırmak, herkesin gördüğünü görmek ama hiç kimsenin düşünmediğini düşünmektir." Ama iş işten geçtikten sonra herkes bilge kesilir! \cite{book:Jammer}
...
Kirchhoff, ``Işık ve ısının yayılımı ve soğurulması arasındaki ilişki" \cite{book:Jammer, Kirchhoff1859} başlıklı makalesinde, etrafları mükemmel yansıtıcı duvarlarla çevrili, yayıcı ve soğurucu özelliklere sahip, sonsuz genişlikte iki opak paralel plakanın ısıl denge durumundaki ışınım alışverişini inceledi. Plakaların ışın yayma, soğurma ve yansıtma özelikleri için sırasıyla $E(\lambda, T)$, $A$ ve $R$ ifadelerini aşağıdaki gibi tanımladı:

\begin{itemize}
\item $E(\lambda, T)$ (spektral yayma gücü): $T$ sıcaklığındaki bir cismin $\lambda$ dalga boyunda, birim yüzey alanı, birim zaman ve birim dalga boyu aralığı başına yaydığı ışıma enerjisini tanımlar ve birimi W/m$^3\equiv \frac{\text{J}}{\text{s}} \frac{1}{\text{m}^3}$'dir.
...
Burada önemli nokta, bir levhanın yaydığı ışınımın tamamının doğrudan diğer levha tarafından soğurulmamasıdır. Her levha, diğer levhanın yaydığı ışınımın yanı sıra sistem içindeki yansımaları ve çevredeki diğer katkıları da içeren toplam denge ışınım alanına maruz kalır. Bir levhaya gelen toplam spektral ışınım $I_\lambda$ ile gösterilirse, levhanın bu ışınımdan soğurduğu kısım $A I_\lambda$ olur.

Termal dengede her levhanın yaydığı spektral enerji, aynı levhanın ortamdan soğurduğu enerjiye eşittir. Bu nedenle
%%
\begin{equation}
	E_1 = A_1 I_\lambda,
	\qquad
...
\end{equation}
%%
yazılır. Buradan aynı sıcaklık ve dalga boyunda
%%
\begin{equation}
	\frac{E_1}{A_1}
	=
...
\end{equation}
%%
sonucu elde edilir. Böylece $E/A$ oranının cismin cinsinden bağımsız, yalnızca dalga boyu ve sıcaklığa bağlı evrensel bir büyüklük olduğu görülür.
...
\caption{Kara cisim, bir kovukta açılan bir delikle temsil edilebilir.}
\label{fig:karacisim}
\end{figure}
...
Böyle bir kara cisim, şekildeki gibi duvarları $T$ sıcaklığında tutulan bir kovukta açılmış küçük bir delikle gerçekleştirilebilir. Deliğe giren elektromanyetik ışınım kovuk içinde çok sayıda yansıma, soğurma ve yeniden yayılma sürecine uğradığından tekrar aynı delikten dışarı çıkma olasılığı çok küçüktür. Bu nedenle küçük açıklık, tüm dalga boylarında yaklaşık olarak tam soğurucu davranır ve ideal bir kara cisme çok iyi bir yaklaşım oluşturur.

Kovuk duvarları $T>0$ sıcaklığında olduklarından yalnızca gelen ışınımı soğurmaz, aynı zamanda termal  ışınım da yayarlar. Duvarlardan yayılan ışınım kovuk içinde tekrar tekrar soğurulur, yansır ve yeniden yayılır. Yeterli süre sonunda kovuk duvarları ile kovuk içindeki elektromanyetik ışınım alanı termal dengeye ulaşır. Bu durumda kovuk içindeki ışınım alanı, duvarların $T$ sıcaklığına karşılık gelen denge spektrumuna sahip olur. Soğurma ve yayma süreçleri mikroskobik düzeyde sürmesine rağmen net enerji alışverişi sıfırdır.


Kirchhoff, termodinamiğin ikinci yasası ve termal denge koşullarından hareketle, kovuk içindeki denge ışınımının homojen ve izotropik olması gerektiğini gösterdi. Homojenlik, belirli bir dalga boyundaki spektral enerji yoğunluğunun kovuk içinde konuma bağlı olmaması; izotropi ise ışınım alanının herhangi bir yönü tercih etmemesi anlamına gelir. Başka bir ifadeyle, kovuk içindeki herhangi bir noktada bütün yönlerden gelen ışınımın istatistiksel dağılımı aynıdır. Ayrıca bu denge ışınımı kovuğun biçiminden ve duvarlarının yapıldığı malzemeden bağımsızdır; aynı sıcaklıktaki bütün ideal kovuklarda her dalga boyunda aynı evrensel ışınım dağılımı elde edilir.
...
Kovuğun içinde $\lambda$ ile $\lambda+d\lambda$ dalga boyları aralığındaki ışımalar için enerji yoğunluğu $w(\lambda,T) d\lambda$'dır ve yayma gücü $E(\lambda,T)$ ile olan ilişkisi aşağıdaki gibidir,
...
%%
%%
Wien kayma yasası olarak bilinen eşitliği bulmuştur. Burada $b = 2.8977729(17)\times10^{-3}$~m~K değerindedir ve biraz önce bahsedilen deneyden elde edilen bir sabittir. Bütün kara cisim spektrumları için geçerlidir. Bu yasa ile belirli bir sıcaklıkta hangi dalga boyunda yayma gücü $E(\lambda,T)$'nin en büyük değere ulaşacağı belirlenebilir. Örneğin Şekil \ref{fig:karaSpektrum}'teki spektrumlar için Wien yasasından faydalanılarak Tablo \ref{tab:wien_shift}'deki değerler bulunur.
%%
%%
\begin{table}[hbtp]
\center
...
\end{equation}
%%
Eğer integral yayma gücü yoğunluğu üzerinden alınmış olsaydı sonuçta toplam birim alan başına güç ($[E(T)]=$ W/m$^2$) ise,
%%
\begin{equation}
...
bu kütle çok küçük olduğundan ışık paketinin (fotonun) hızının neredeyse ışık hızı $c$'ye çok yakın olması gerektiğini öne sürmüştür. Böylece bir fotonun bütün enerjisi için,
\begin{equation}
...
\end{equation*}
%%
iken, Proton'un yarı çapı $10^{-13}$~cm'dir. Bu nedenle günlük yaşantımızda ve deneylerimizde makroskopik seviyede dalga etkilerini gözleyemeyiz \cite{book:Gasiorowicz}. 

%Işığın veya elektromanyetik dalganın parçacık yapısı davranışının gözlenebilmesi için ise bir ışık paketinin dalga boyu ve momentumunun çarpımı Planck sabiti civarında olmalıdır ($\lambda p \eqsim h$).
...
% Tonomura double slit experiment with electrons
...
\begin{figure}[hbtp]
\center
\includegraphics[scale=.35]{Thomson_vs_Rutherford_Atom.png}
...
\begin{minipage}{.48\textwidth}
\center
\includegraphics[scale=.5]{Hydrogen_transitions.png}
...
%     broadly : the negative electrode of a diode
%— compare ANODE
...
%%\href{https://en.wikipedia.org/wiki/Franck%E2%80%93Hertz_experiment}{Wikipedia}
...
\begin{figure}[hbtp]
	...
	\begin{minipage}{.3\textwidth}
		\center
		\includegraphics[scale=.8]{FranckHertz_schema.png}
	...
	\begin{minipage}{.48\textwidth}
		\center
		\vspace{12pt}
		\includegraphics[scale=.4]{Franck_Hertz_tube_horizontal.png}
		\vspace{6pt}
...
\begin{figure}[hbtp]
	...
	\begin{minipage}{.48\textwidth}
		\center
		\includegraphics[scale=.5]{Franck_Hertz_Volt_v_Current.png}
	...
	\begin{minipage}{.48\textwidth}
		\center
		\includegraphics[scale=.38]{Franck_Hertz_Mercury_spectrum.png}
...
%%Bir atomun dairesel yörüngesindeki bir elektronun klasik ve kuantum %%davranışları $n+1 \longrightarrow n$ geçişleri uygunluk prensiyle uyumluyken,
%%$n+2 \longrightarrow n$ geçişi için 
